...
Install macOS `launchd` background services:

```bash
./install_launchd.sh
```

Remove macOS `launchd` background services:
...
After `./install_launchd.sh`, the existing `./start.sh`, `./stop.sh`, and `./status.sh` commands continue to work, but they operate through `launchd`.
